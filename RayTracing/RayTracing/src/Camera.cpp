  ...
  if (delta.x != 0.f || delta.y != 0.f) {
    ...
    glm::quat q = glm::normalize(
        glm::cross(glm::angleAxis(-pitchDelta, rightDirection),
                   glm::angleAxis(-yawDelta, glm::vec3(0.f, 1.f, 0.f))));
...
void Camera::RecalculateRayDirections() {
  ...
  for (uint32_t y = 0; y < m_ViewportHeight; y++) {
    for (uint32_t x = 0; x < m_ViewportWidth; x++) {
      glm::vec2 coord = {x / (float)m_ViewportWidth,
                         y / (float)m_ViewportHeight};
      ...
      glm::vec3 rayDirection =
          glm::vec3(m_InverseView *
                    glm::vec4(glm::normalize(glm::vec3(target) / target.w),
                              0)); // World Space
